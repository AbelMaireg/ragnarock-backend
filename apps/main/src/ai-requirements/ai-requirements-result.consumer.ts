...
import {
  ...
  AiPlannerResultEvent,
  AiQaIntelligenceResultEvent,
  AiRequirementsResultEvent,
  ...
  private async processMessage(messageId: string, payload: string): Promise<void> {
    ...
    try {
      if (result.status === "arch_doc_succeeded" || result.status === "arch_doc_failed") {
        ...
        result.status === "qa_intelligence_succeeded" ||
        result.status === "qa_intelligence_failed"
      ) {
        await this.qaIntelligenceService.handleResult(result as AiQaIntelligenceResultEvent);
      } else if (
        result.status === "ragnarock_chat_succeeded" ||
        result.status === "ragnarock_chat_failed"
      ) {
